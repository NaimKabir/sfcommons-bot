...
function postPrivateInitiation(memberIds: Array<string>) {
  const tagGroup = (memberIds: Array<string>) => memberIds.map( (id) => `<@${id}>`).join(' ')
  ...
  CLIENT.conversations.open({
    ...
    (result) => {
      if (result.channel && result.channel.id) {
        ...
        .catch((e) => handleError("Failed to post message to private conversation.", {memberIds: memberIds, errorResponse: e}))
      } else {
  ...
  .catch((e) => handleError("Failed to open private conversation.", {memberIds: memberIds, errorResponse: e}))
}
...
getGroupedMemberIDs()
  .then( (groups) => {
    groups.forEach( 
      (memberIds) => postPrivateInitiation(memberIds)
    )
  })
  .catch((e) => handleError("Failed to get groups of memberIds", {errorResponse: e} ))